...
export default function App() {
  ...
  async function addTransaction(tx: Omit<Transaction, 'id' | 'user_id' | 'created_at'>) {
    ...
    if (!error && data) {
      ...
      await supabase.from('gamification').update({
        ...
        last_record_date: new Date().toISOString().split('T')[0]
      }).eq('user_id', user!.id)
  ...
  async function addCategory(cat: Omit<Category, 'id' | 'user_id'>) {
  ...
  const navItems = [
    { id: 'dashboard', label: 'Inicio', icon: 'M3 3h7v7H3zM14 3h7v7h-7zM3 14h7v7H3zM14 14h7v7h-7z' },
    { id: 'movimientos', label: 'Movimientos', icon: 'M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01' },
    { id: 'consolidado', label: 'Consolidado', icon: 'M3 3h7v7H3zM14 3h7v7h-7zM3 14h7v7H3zM14 14h7v7h-7z' },
    { id: 'presupuesto', label: 'Presupuesto', icon: 'M3 3h18v18H3zM3 9h18M9 21V9' },
    { id: 'reportes', label: 'Reportes', icon: 'M18 20V10M12 20V4M6 20v-6' },
  ]

  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: '#0d0d14', fontFamily: "'DM Sans', sans-serif", colorScheme: 'dark' }}>
        ...
        <div style={{ margin: '8px 10px 4px', background: '#18182a', borderRadius: '8px', padding: '3px', display: 'flex', border: '1px solid #2a2a3e' }}>
          {(['personal', 'empresa'] as Space[]).map(s => (
            <button key={s} onClick={() => setSpace(s)} style={{
              flex: 1, padding: '5px 0', textAlign: 'center', borderRadius: '5px',
              fontSize: '10px', fontWeight: 600, cursor: 'pointer', transition: 'all .2s',
              border: 'none', fontFamily: "'DM Sans', sans-serif",
              background: space === s ? 'linear-gradient(135deg,#8b7ff0,#6a8af0)' : 'transparent',
              color: space === s ? '#fff' : '#7070b0'
        ...
        <nav style={{ flex: 1, overflowY: 'auto', padding: '6px 8px' }}>
          {navItems.map(item => (
            <button key={item.id} onClick={() => setPage(item.id)} style={{
              ...
              padding: '8px 9px', borderRadius: '9px', cursor: 'pointer',
              transition: 'all .15s', fontSize: '12px', fontWeight: 500,
              marginBottom: '2px', border: 'none', width: '100%', textAlign: 'left',
              fontFamily: "'DM Sans', sans-serif",
              background: page === item.id ? 'rgba(139,127,240,.2)' : 'transparent',
              color: page === item.id ? '#b0a8ff' : '#8888b8'
            }}>
          ...
          <button onClick={() => setPage('ajustes')} style={{
            display: 'flex', alignItems: 'center', gap: '8px',
            padding: '8px 9px', borderRadius: '9px', cursor: 'pointer',
            fontSize: '12px', fontWeight: 500, marginBottom: '2px',
            border: 'none', width: '100%', textAlign: 'left',
            fontFamily: "'DM Sans', sans-serif",
            background: page === 'ajustes' ? 'rgba(139,127,240,.2)' : 'transparent',
            color: page === 'ajustes' ? '#b0a8ff' : '#8888b8'
          }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div style={{ flex: 1, overflowY: 'auto', background: '#0d0d14' }}>
        {loading && page !== 'ajustes' ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#6060a0' }}>
